...
void write_time_log(char *filename, char *data, int data_include)
{
	...
	if (filename == NULL)
		if (!mfts_enable)
			...
			fname = "/data/touch/touch_self_test_mfts.txt";
	else
		fname = filename;
...
static int print_swipe_fail_reason(struct synaptics_ts_data *ts)
{
	...
	synaptics_ts_page_data_read(ts->client, LPWG_PAGE,
			swp->swipe_fail_reason_reg, 1, &buf);
...
static int tci_control(struct synaptics_ts_data *ts, int type, u8 value)
{
	...
	case REPORT_MODE_CTRL:
		...
		if (ts->ts_swipe_data.support_swipe == SWIPE_VER_1) {
...
static int swipe_enable(struct synaptics_ts_data *ts)
{
	...
	if (swp->support_swipe == SWIPE_VER_2) {
		DO_SAFE(synaptics_ts_set_page(client, LPWG_PAGE), error);
	}
	...
	} else if (swp->support_swipe == SWIPE_VER_1) {
		DO_SAFE(touch_i2c_read(client, swp->swipe_min_distance_reg, 19, buf), error);
		buf[18] = swp->swipe_min_distance;
		DO_SAFE(touch_i2c_write(client, swp->swipe_min_distance_reg, 19, buf), error);
...
static int sleep_control(struct synaptics_ts_data *ts, int mode, int recal)
{
	...
	if(mode == 3) {	/* in this case, reset Touch IC for sensor reset -> IC Reset */
		...
		if (!strncmp(ts->fw_info.fw_product_id, "PLG446", 6) || !strncmp(ts->fw_info.fw_product_id, "PLG468", 6)) {
			if (((curr & 0xF8) == DEVICE_CONTROL_SLEEP) && !mode) { /* curr - sleep, next - sleep */
				TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : It's odd case (sleep)(sleep)\n", __func__);
				return 0;
			}
			next = (curr & 0xF8) | mode ?
				DEVICE_CONTROL_NOSLEEP : DEVICE_CONTROL_SLEEP;
		} else {
			next = (curr & 0xFC) | mode ?
...
void get_finger_amplitude(struct synaptics_ts_data *ts)
{
	...
	TOUCH_INFO_MSG("saturation_cap = 0x%04X\n", saturation_cap);
...
static int get_binFW_version(struct synaptics_ts_data *ts)
{
	...
	rc = request_firmware(&fw_entry,
		...
		&ts->client->dev);
	if (rc != 0) {
		TOUCH_ERR_MSG("[%s] request_firmware() failed %d\n", __func__, rc);
		return -EIO;
	}
...
static ssize_t show_firmware(struct i2c_client *client, char *buf)
{
	...
	if (!(strncmp(ts->fw_info.fw_image_product_id, "PLG349", 6))) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Touch IC : s3528\n");
	} else if (!(strncmp(ts->fw_info.fw_image_product_id, "s3320", 5))
	...
	case CHARGERLOGO_MODE:
		...
		if(!(strncmp(ts->fw_info.fw_product_id, "PLG446", 6))) {
			...
			DO_SAFE(synaptics_ts_page_data_read(client, ANALOG_PAGE,
					0x54, 1, buffer), error);
			if(!buffer[0]) {
				TOUCH_INFO_MSG("%s: DDIC Control bit cleared.\n",
				__func__);
			}
	...
	case NORMAL_BOOT_MODE:
		...
		if(!(strncmp(ts->fw_info.fw_product_id, "PLG446", 6))) {
			...
			DO_SAFE(synaptics_ts_page_data_read(client, ANALOG_PAGE,
					0x54, 1, buffer), error);
			if(buffer[0]) {
				TOUCH_INFO_MSG("%s: DDIC Control bit set 1 again.\n",
				__func__);
			}
		}
...
error:
	mutex_unlock(&ts->pdata->thread_lock);
	TOUCH_ERR_MSG("%s, failed DDIC Control\n",
			__func__);
	return count;
...
static int get_swipe_info(struct synaptics_ts_data *ts)
{
	...
	if (!strcmp(ts->fw_info.fw_product_id, "PLG349")) {
		...
		TOUCH_INFO_MSG("%s: is_official_fw=%d, fw_ver=%d\n",
				__func__, is_official_fw, fw_ver);

		if (is_official_fw) {
			if ((fw_ver >= 5) && (fw_ver <= 6)) {
				swp->support_swipe = SWIPE_VER_1;
			} else if (fw_ver >= 7) {
				swp->support_swipe = SWIPE_VER_2;
			} else {
	...
	if (swp->support_swipe > NO_SUPPORT_SWIPE) {
		...
		swp->swipe_coordinate_start_reg = ts->f51.dsc.data_base + 73;
		swp->swipe_coordinate_end_reg = ts->f51.dsc.data_base + 77;
		swp->swipe_fail_reason_reg = ts->f51.dsc.data_base + 81;
		swp->swipe_time_reg = ts->f51.dsc.data_base + 82;
	}

	if (swp->support_swipe == SWIPE_VER_1) {
		swp->swipe_enable_mask = 0x02;
		swp->swipe_gesture = 0x07;
		swp->swipe_enable_reg = ts->f12_reg.ctrl[27];
		swp->swipe_min_distance_reg = ts->f12_reg.ctrl[18];
		swp->swipe_ratio_threshold_reg = ts->f51.dsc.control_base + 16;
		swp->swipe_ratio_check_period_reg = ts->f51.dsc.control_base + 17;
		swp->min_swipe_time_thres_lsb_reg = ts->f51.dsc.control_base + 18;
		swp->min_swipe_time_thres_msb_reg = ts->f51.dsc.control_base + 19;
		swp->max_swipe_time_thres_lsb_reg = ts->f51.dsc.control_base + 20;
		swp->max_swipe_time_thres_msb_reg = ts->f51.dsc.control_base + 21;
	}
	...
	if (swp->support_swipe >= SWIPE_VER_2) {
		...
		swp->swipe_enable_reg = ts->f51.dsc.control_base + 16;
		swp->swipe_min_distance_reg = ts->f51.dsc.control_base + 17;
		swp->swipe_ratio_threshold_reg = ts->f51.dsc.control_base + 18;
		swp->swipe_ratio_check_period_reg = ts->f51.dsc.control_base + 19;
		swp->swipe_ratio_check_min_distance_reg = ts->f51.dsc.control_base + 20;
		swp->min_swipe_time_thres_lsb_reg = ts->f51.dsc.control_base + 21;
		swp->min_swipe_time_thres_msb_reg = ts->f51.dsc.control_base + 22;
		swp->max_swipe_time_thres_lsb_reg = ts->f51.dsc.control_base + 23;
		swp->max_swipe_time_thres_msb_reg = ts->f51.dsc.control_base + 24;
	}
	...
	TOUCH_INFO_MSG("swipe_fail_reason_reg = 0x%02X\n", swp->swipe_fail_reason_reg);
...
static int get_ic_info(struct synaptics_ts_data *ts)
{
	...
	if (ts->pdata->role->bootloader_type == BL_VER_HIGHER){
		DO_SAFE(touch_i2c_read(ts->client, FLASH_CONFIG_ID_REG,
				sizeof(temp_pid) - 1,
				temp_pid), error);

		TOUCH_DEBUG(DEBUG_BASE_INFO, "[%s] Bootloader Type = %d \n", __func__, ts->pdata->role->bootloader_type);

                memset(ts->fw_info.fw_product_id, 0, sizeof(ts->fw_info.fw_product_id)/sizeof(u8));
		memcpy(ts->fw_info.fw_product_id, &temp_pid[4], 6);

		TOUCH_INFO_MSG("[%s] Product_ID_Reset ! , addr = 0x%x, P_ID = %s \n",__func__,
			FLASH_CONFIG_ID_REG ,ts->fw_info.fw_product_id);
	}
		...
		const struct touch_data *prev_data)
{
	...
	DO_SAFE(touch_i2c_read(client, ts->f54_reg.interference__metric_LSB, 1, &buf1), error);
	DO_SAFE(touch_i2c_read(client, ts->f54_reg.interference__metric_MSB, 1, &buf2), error);
	im = (buf2 << 8) | buf1;
	im_sum += im;

	DO_SAFE(touch_i2c_read(client, ts->f54_reg.current_noise_status, 1, &cns), error);
	cns_sum += cns;

	if (!ts->pdata->panel_id) {
		DO_SAFE(touch_i2c_read(client, ts->f54_reg.cid_im, 2, buffer), error);
		cid_im = (buffer[1]<<8)|buffer[0];
		cid_im_sum += cid_im;
	}

	DO_SAFE(touch_i2c_read(client, ts->f54_reg.freq_scan_im, 2, buffer), error);
...
error:
	 TOUCH_ERR_MSG("%s, %d : get ts noise failed\n", __func__, __LINE__);
	return -EPERM;
}
		...
		const struct touch_data *prev_data)
{
	...
	if (ts->ts_data.interrupt_status_reg & int_mask_cust) {
		...
		if (ts->ts_swipe_data.support_swipe == SWIPE_VER_1) {
			...
			DO_SAFE(touch_i2c_read(client, ts->f12_reg.data[4], 1, &swipe_buf), error);
...
/*
	if(ts->pdata->role->bootloader_type == BL_VER_HIGHER) {
		TOUCH_DEBUG(DEBUG_BASE_INFO,
					"%s : Type of Boot loader is Ver. 7.2, So will ignore FW upgrade\n", __func__);
		return NO_UPGRADE;
	}
*/
	if (ts->pdata->inbuilt_fw_name != NULL && !info->fw_force_upgrade) {
	...
	if(!(strcmp(ts->fw_info.fw_product_id, "s3320")) || /* Force Upgrade for P1 on 1st cut temporarily */
		!(strncmp(ts->fw_info.fw_image_product_id, "PLG468", 6)))
		need_upgrade = 1;

	TOUCH_INFO_MSG("[%s] fw_image_product_id : %s\n",__func__, ts->fw_info.fw_image_product_id);
	...
	TOUCH_INFO_MSG("ts_need_upgrade = %d, need_upgrade = %d \n", ts->pdata->fw->need_upgrade, need_upgrade);
	...
	if (need_upgrade || ts->fw_info.need_rewrite_firmware) {
		...
		if (info->fw_path != NULL) {
			TOUCH_INFO_MSG("FW: need_upgrade[%d] force[%d] file[%s]\n",
				fw->need_upgrade,
				info->fw_force_upgrade, info->fw_path);
			goto firmware_up;
		} else {
			goto error;
		}
