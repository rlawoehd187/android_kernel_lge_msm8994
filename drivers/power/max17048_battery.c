...
static void external_charger_enable(bool enable)
{
	...
	val.intval = enable;
	ref->batt_psy->set_property(ref->batt_psy, POWER_SUPPLY_PROP_BATFET_EN, &val);
}
#endif
...
static void max17048_work(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s config : 0x%x /status : 0x%x. \n", __func__, chip->config, chip->status);
#endif
	...
	printk(KERN_ERR "%s : Capacity : %d,  Voltage : %d, raw soc : %d.%1d, ocv : %d\n",
